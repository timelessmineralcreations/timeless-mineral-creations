...
async function updateBirthstone(formData) {
  "use server";
  await requireAdmin();
  ...
  if (!id) {
    throw new Error("Birthstone ID is missing.");
  }
  ...
  const duplicateBirthstone = await prisma.birthstone.findFirst({
    where: {
      slug,
      NOT: {
        id,
  ...
  revalidatePath(`/admin/birthstones-catalog/${id}`);

  redirect("/admin/birthstones-catalog");
}

async function deleteBirthstone(formData) {
  "use server";
  await requireAdmin();
  ...
  await prisma.birthstone.delete({
    where: {
      id,
    },
  });
...
export default async function EditBirthstonePage({ params }) {
  const { id } = await params;

  const birthstone = await prisma.birthstone.findUnique({
    where: {
      id,
    },
  });
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Update the birthstone image, month, supplier information,
            pricing, and availability.
          </p>
        </div>

        <form action={updateBirthstone}>
          <input
            type="hidden"
            name="id"
            value={birthstone.id}
          ...
          >
            <FormSection
              ...
              description="Edit the name, URL slug, birth month, and customer-facing description."
            >
              <div style={twoColumnGridStyle}>
                ...
                <Field
                  label="URL Slug"
                  helpText="Used in the admin and future product links."
                >
                  <input
                    name="slug"
                    type="text"
                    defaultValue={birthstone.slug}
                    style={inputStyle}
                  />
                </Field>
              </div>
              ...
              <Field label="Birth Month" required>
                <select
                  name="monthNumber"
                  required
                  defaultValue={birthstone.monthNumber}
                  style={inputStyle}
                >
                  {months.map((month) => (
                    <option
              ...
              <Field label="Description">
                <textarea
                  name="description"
                  rows={5}
                  defaultValue={birthstone.description || ""}
                  style={{
                    ...inputStyle,
                    minHeight: "140px",
                    resize: "vertical",
            ...
            <FormSection
              title="Stone Details"
              description="Edit the stone type, color, shape, and size."
            >
              <div style={twoColumnGridStyle}>
                <Field label="Stone Type">
                  <select
                    name="stoneType"
                    defaultValue={birthstone.stoneType || "CZ"}
                    style={inputStyle}
                  >
                ...
                <Field label="Color Name">
                  <input
                    name="colorName"
                    type="text"
                    defaultValue={birthstone.colorName || ""}
                    style={inputStyle}
                  />
                </Field>
              </div>
            ...
            <FormSection
              title="Appearance"
              description="Replace the saved image and update the approximate display color."
            >
              <ImageUploader
                name="imageUrl"
                label="Birthstone Image"
                folder="birthstones"
                defaultValue={birthstone.imageUrl || ""}
              ...
              >
                <input
                  name="colorHex"
                  type="color"
                  defaultValue={birthstone.colorHex || "#9b1c31"}
                  style={{
                    ...inputStyle,
                    padding: "6px",
                    cursor: "pointer",
            ...
            <FormSection
              ...
              description="Update the supplier source and your cost for the birthstone."
            >
              <div style={twoColumnGridStyle}>
                <Field label="Supplier">
                  <input
                    name="supplier"
                    type="text"
                    defaultValue={birthstone.supplier || ""}
                    style={inputStyle}
                  />
                </Field>
                ...
                <Field label="Supplier URL">
                  <input
                    name="supplierUrl"
                    type="url"
                    defaultValue={birthstone.supplierUrl || ""}
                    style={inputStyle}
                  />
                </Field>
              </div>
              ...
              <Field label="Supplier Cost">
                ...
                >
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    defaultValue={(birthstone.supplierCostCents / 100).toFixed(2)}
                    style={{
                      ...inputStyle,
                      paddingLeft: "32px",
                    }}
            ...
            <FormSection
              title="Pricing"
              description="Update any additional amount charged when this birthstone is selected."
            >
              <Field label="Price Adjustment">
                ...
                >
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    defaultValue={(birthstone.priceAdjustmentCents / 100).toFixed(2)}
                    style={{
                      ...inputStyle,
                      paddingLeft: "32px",
                    }}
            ...
            >
              ...
              >
                <input
                  name="sortOrder"
                  type="number"
                  defaultValue={birthstone.sortOrder}
                  style={inputStyle}
                />
              </Field>
              ...
              >
                <CheckboxField
                  ...
                  defaultChecked={birthstone.active}
                />
                ...
                <CheckboxField
                  ...
                  defaultChecked={birthstone.featured}
                />
              </div>
            </FormSection>
