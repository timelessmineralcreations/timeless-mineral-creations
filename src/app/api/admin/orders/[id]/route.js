...
export async function PATCH(request, { params }) {
  try {
    ...
    if (
      body.incomingCarrier !== undefined
    ) {
      data.incomingCarrier =
        cleanOptionalString(
          body.incomingCarrier
        );
    }
    ...
    if (body.materialsReceived === true) {
      data.materialsReceivedAt =
        existingOrder.materialsReceivedAt ||
        new Date();
    ...
    if (body.materialsVerified === true) {
      data.materialsVerifiedAt =
        existingOrder.materialsVerifiedAt ||
        new Date();
    }
    ...
    if (
      body.outgoingCarrier !== undefined
    ) {
      data.outgoingCarrier =
        cleanOptionalString(
          body.outgoingCarrier
        );
    }
    ...
    if (body.outgoingShipped === true) {
      data.outgoingShippedAt =
        existingOrder.outgoingShippedAt ||
        new Date();
    ...
    if (
      body.materialsPrepared === true
    ) {
      data.materialsPreparedAt =
        existingOrder.materialsPreparedAt ||
        new Date();
    ...
    if (body.buildCompleted === true) {
      data.buildCompletedAt =
        existingOrder.buildCompletedAt ||
        new Date();
    ...
    if (
      body.engravingCompleted === true
    ) {
      data.engravingCompletedAt =
        existingOrder.engravingCompletedAt ||
        new Date();
    }

    if (
      body.engravingCompleted === false
    ) {
    ...
    if (body.qualityChecked === true) {
      data.qualityCheckedAt =
        existingOrder.qualityCheckedAt ||
        new Date();
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "No valid order changes were provided.",
        },
        { status: 400 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    /*
     * Require both the word DELETE and
     * the matching order ID so an
     * accidental request cannot remove
     * an order.
     */
    if (
    ...
    /*
     * OrderItem and Refund records are
     * configured with onDelete: Cascade,
     * so Prisma will clean them up with
     * the order.
     *
     * This does NOT refund Stripe and
     * does NOT void an EasyPost label.
     */
